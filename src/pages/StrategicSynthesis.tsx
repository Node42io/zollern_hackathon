...
export default function StrategicSynthesis() {
  ...
  return (
    <>
      ...
      <section id="syn-investments" className="container" style={{ paddingTop: 48 }}>
        <SectionAnchor id="syn-investments" title="Investment Priorities" kicker="Ranked by ROI" />
        <div className="md">
          <p style={{ marginBottom: 20 }}>
            9 investments ranked by return speed and breadth of market served. Investments 1–4 total less than €500K and collectively serve all 26 markets.
          </p>
          <div style={{
            background: "var(--bg-card)",
            border: "1px solid var(--border-subtle)",
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "rgba(255,255,255,0.02)", borderBottom: "1px solid var(--border-subtle)" }}>
                  {["#", "Investment", "Cost", "Markets Served", "Payback"].map((h) => (
                    <th key={h} style={{
                      padding: "12px 16px",
                      textAlign: "left",
                      fontFamily: "var(--font-mono)",
              ...
              <tbody>
                {investmentPriorities.map((inv, i) => {
                  const isEarlyWin = inv.rank <= 4;
                  return (
                    <tr key={inv.rank} style={{
                      borderBottom: i < investmentPriorities.length - 1 ? "1px solid var(--divider)" : undefined,
                      background: isEarlyWin ? "rgba(111,213,155,0.02)" : undefined,
                    }}>
                      <td style={{
                        padding: "13px 16px",
                        fontFamily: "var(--font-mono)",
                        fontSize: 14,
                        fontWeight: 700,
                        color: isEarlyWin ? "#6fd59b" : "var(--text-gray)",
                        textAlign: "center",
                        width: 40,
                      }}>
                        {inv.rank}
                      </td>
                      <td style={{ padding: "13px 16px", color: "var(--text-white)", fontWeight: isEarlyWin ? 500 : 400 }}>
                        {inv.investment}
                      </td>
                      <td style={{
                        padding: "13px 16px",
                        fontFamily: "var(--font-mono)",
                        fontSize: 12,
                        color: isEarlyWin ? "#6fd59b" : "var(--text-gray-light)",
                        whiteSpace: "nowrap",
                      }}>
                        {inv.cost}
                      </td>
                      <td style={{ padding: "13px 16px", color: "var(--text-gray-light)", fontSize: 12 }}>
                        {inv.marketsServed}
                      </td>
                      <td style={{
                        padding: "13px 16px",
      ...
      {/* ── 7. Revenue Scenarios ─────────────────────────────────────── */}
      <section id="syn-revenue" className="container" style={{ paddingTop: 48, paddingBottom: 64 }}>
        <SectionAnchor id="syn-revenue" title="Revenue Scenarios" kicker="3 Tiers × 3 Scenarios" />
        <div className="md">
          <p style={{ marginBottom: 24 }}>
            Total addressable incremental revenue from conservative execution (Tier 1 only, low win rate) to optimistic (all tiers, high win rate). Base scenario assumes ~50% of identified opportunities captured at midpoint revenue estimates.
          </p>
          ...
          <div style={{
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "rgba(255,255,255,0.02)", borderBottom: "1px solid var(--border-subtle)" }}>
                  <th style={{ padding: "12px 20px", textAlign: "left", fontFamily: "var(--font-mono)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--text-gray-dark)", fontWeight: 500 }}>
                    Tier / Scenario
                  </th>
                  {["Conservative", "Base Case", "Optimistic"].map((s) => (
                    <th key={s} style={{
                      padding: "12px 20px",
                      textAlign: "center",
                      fontFamily: "var(--font-mono)",
                      ...
                      textTransform: "uppercase",
                      letterSpacing: "0.08em",
                      color: s === "Base Case" ? "var(--accent-yellow)" : "var(--text-gray-dark)",
                      fontWeight: 500,
                    }}>
                      {s}
                    </th>
              ...
              <tbody>
                {revenueScenarios.tiers.map((tier) => (
                  <tr key={tier.tier} style={{ borderBottom: "1px solid var(--divider)" }}>
                    <td style={{ padding: "16px 20px" }}>
                      <div style={{ fontWeight: 500, color: "var(--text-white)", marginBottom: 2 }}>
                        {tier.label}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--text-gray)", fontFamily: "var(--font-mono)" }}>
                        {tier.subtitle}
                      </div>
                    </td>
                    {[tier.conservative, tier.base, tier.optimistic].map((val, j) => (
                      <td key={j} style={{
                        padding: "16px 20px",
                        textAlign: "center",
                        fontFamily: "var(--font-mono)",
                        ...
                        color: j === 0 ? "var(--text-gray-light)" : j === 1 ? "var(--accent-yellow)" : "var(--text-white)",
                      }}>
                        {val}
                      </td>
                    ))}
                ...
                <tr style={{ background: "rgba(255,255,255,0.02)" }}>
                  <td style={{ padding: "18px 20px" }}>
                    <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-gray-dark)", marginBottom: 4 }}>
                      Total (All Tiers)
                    </div>
                    <div style={{ fontSize: 11, color: "var(--text-gray)" }}>
                      vs. ZOLLERN Group ~€385M
                    </div>
                  </td>
                  {[
                    { val: revenueScenarios.totals.conservative, bold: false },
                    { val: revenueScenarios.totals.base, bold: true },
                    { val: revenueScenarios.totals.optimistic, bold: false },
                  ].map((t, j) => (
                    <td key={j} style={{
                      padding: "18px 20px",
                      textAlign: "center",
                      ...
                      {j === 1 && (
                        <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--text-gray)", marginTop: 4 }}>
                          ~27% incremental uplift
                        </div>
                      )}
                    </td>
                  ))}
          ...
          <div style={{
            ...
            <p style={{ fontSize: 14, color: "var(--text-gray-light)", lineHeight: 1.6, margin: 0 }}>
              ...
              <strong style={{ color: "var(--text-white)" }}>selective certification</strong>. These are solvable with focused management attention and €500K–1M of initial investment — a fraction of the €103M base-case revenue opportunity.
            </p>
          </div>
        </div>
      </section>
